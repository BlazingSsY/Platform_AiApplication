<script setup lang="ts">
  ...
  const feedbackPlaceholderMap: Record<string, string> = {
    '规则不适用': '示例: 该文件为汇编内联头文件，审查规则要求C语言变量声明，在此不适用。',
    '问题可例外': '示例: 该goto语句仅用于统一跳转到函数末尾释放资源，符合内核编码惯例。',
    '复查无影响': '示例: 报告的未使用变量仅存在于调试宏中，release编译时已被预处理移除。',
    '审查不正确': '示例: 该指针转换经由void*中转且目标类型与原类型兼容，审查工具误判了类型违例。',
  }
  ...
  const submitAudit = async () => {
    ...
    try {
      ...
      const res: any = await sourceApi.sourceCodeReviewRecheck(param)
      if (res.succ) {
        // 关闭弹窗
  ...
  const router = useRouter()
    ...
    if(path&&path===`/reviewDetail`){
      ...
      router.push({ path: `/sourceCodeDocumentReview` })
    }
  }
  ...
  const sourceCodeFileVersionFn = (fileId: any) => {
    sourceApi.sourceCodeFileVersion({ fileId }).then(res => {
      if (res.succ) {
        versionOptions.value = res.content
      }
    })
  }
  const sourceCodeFileResultFn = async (versionId: any) => {
    const res = await sourceApi.sourceCodeFileResult({ versionId })
    if (res.succ) {
  ...
  const initDataFn = () => {
    ...
    //   sourceCodeFileResultFn(versionFilters.version)
    // }
  ...
  const getDetailFn = async versionId => {
    const res = await sourceApi.sourceCodeFileResult({ versionId, includeAllStatus: true })
    if (res.succ) {
  ...
  if (item) {
    ...
    // } else {
      // sourceCodeFileVersionFn(rowItem.value.id)
