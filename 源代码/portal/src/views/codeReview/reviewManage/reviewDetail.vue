<script setup lang="ts">
  ...
  import CodeDisplayDialog from '@/views/codeReview/components/CodeDisplay/CodeDisplayDialog.vue';
    import {
    sourceCodeReviewRecheckDetail,
    sourceCodeReviewRecheckResultSubmit
  ...
  const fetchReviewResultDetails = async (page = 1, pageSize = 10, noLoading?) => {
    ...
    try {
      const res = await sourceCodeReviewRecheckDetail({"fileVersionId": rowItem.value.fileVersionId})
      if (res.succ) {
  ...
  const handleApprove = (list,type) => {
    if (type === 'APPROVE') {
      ElMessageBox.confirm(`确认通过复核吗？`, '提示', {
        ...
        sourceCodeReviewRecheckResultSubmit(sentData).then(res=>{
          if (res.succ) {
