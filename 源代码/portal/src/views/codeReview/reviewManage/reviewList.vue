<script setup >
  ...
  import {
    sourceCodeReviewRecheckPage
  } from "@/ajax/sourcecodereview"
  ...
  const fetchCircuitFiles = async (page = 1, pageSize = 10) => {
    ...
    const res = await sourceCodeReviewRecheckPage(params)
    if (res.succ) {
      // 直接使用新的数据结构
      ...
      <div class="tableBox">
        <div class="search">
          <el-form ref="ruleFormRef" :inline="true">
            <el-form-item label="代码文件名">
              <el-input v-model="searchObj.fileName" placeholder="输入代码文件名" style="width: 240px" clearable />
            </el-form-item>
            <el-form-item v-if="roleName && roleName !== `普通用户`" label="单位">
              <el-select v-model="searchObj.depId" placeholder="选择单位" style="width: 240px" filterable clearable @change="depChange">
            ...
            <el-form-item v-if="roleName && roleName !== `普通用户`" label="用户">
              <el-select v-model="searchObj.userId" :disabled="!searchObj.depId" placeholder="选择用户" style="width: 240px" filterable clearable>
        ...
        <div class="tableWrapper">
          ...
          >
            ...
            <el-table-column prop="fileName" label="代码文件" align="left" show-overflow-tooltip sortable="custom" min-width="200px" />
            ...
            <el-table-column prop="status" label="状态" align="center" min-width="100px" sortable="custom">
              <template #default="scope">
                {{ scope.row.recheckStatus===1?`未复核`:scope.row.recheckStatus===2?`复核中`:`已复核` }}
              </template>
            </el-table-column>
            <el-table-column prop="time" label="复核提交时间" align="center" min-width="160px" sortable="custom">
              <template #default="scope">
                {{ scope.row.time || `-` }}
              </template>
            </el-table-column>
            <el-table-column prop="ownerName" label="隶属人" align="center" min-width="100px" sortable="custom">
              <template #default="scope">
                {{ scope.row.ownerName || `-` }}
              </template> 
            </el-table-column>
            <el-table-column label="操作" align="center" width="140" class-name="opt" fixed="right">
              <template #default="scope">                                          
                <el-button type="primary" size="small" class="btn" @click="detailFn(scope.row)"> 查看详情 </el-button>
              </template>
            </el-table-column>
          </el-table>
        </div>
          ...
          <div class="tableCont">
            <el-table ref="rulesListRef" border tooltip-effect="light" :data="viewVersionListData" height="100%" max-height="500px" >
              ...
              <el-table-column label="操作" align="center" width="120" class-name="opt" fixed="right">
                  <template #default="scope">
                      <el-button type="primary" size="small" class="btn" @click="detailFn(scope.row.id,rowData.fileName,scope.row.fileVersionName,rowData.auditSubmitTime,scope.row.fileId,scope.row.fileVersionId,rowData.isRecycle)"> 查看详情 </el-button>
                  </template>
              </el-table-column>
            </el-table>
          </div>
