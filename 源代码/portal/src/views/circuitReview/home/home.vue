  ...
  >
    ...
    <div class="chart-section">
      ...
      >
        <div class="label">
          <div class="icon" />
          <div class="text">{{ isStaff ? '前12个月文件及审图数量统计' : '审图数量统计' }}</div>
        </div>
        <div class="review-chart-container">
          <last-time-chart
            v-if="isStaff"
            :dataset="reviewChartData"
          />
          <review-chart  
            v-else
            :dataset="reviewChartData"
      ...
      >
        <div class="label">
          <div class="icon" />
          <div class="text">{{ isStaff ? '文件通过率统计' : '用户数量统计' }}</div>
        </div>
        <div class="user-chart-container">
          <user-chart :dataset="userChartData" :title="isStaff ? '文件通过率统计' : '用户数量统计'" />
        </div>
      </div>
      <div
        :class="[
...
import LastTimeChart from "./last-time-chart/LastTimeChart.vue"
...
.review-entry {
  width: 780px;
  height: 245px;
  background: url(@/assets/images/circuitReview/entryNew1.png) no-repeat center center;
