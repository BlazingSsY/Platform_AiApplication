...
import { getToolPages, toolFileUploadFile, deleteToolFile } from "@/ajax/circuitreview"
...
<template>
  <div v-loading.fullscreen.lock="loading" class="toolDownload" element-loading-background="rgba(122, 122, 122, 0.8)" :element-loading-text="loadingText">
    <localTitle title="工具下载列表">
      <template #rightBtn>
        <el-button v-if="roleInfo?.name === `管理员`" type="primary" icon="Upload" style="margin-left: 12px" @click="uploadFn">上传工具</el-button>
      </template>
    </localTitle>
    <div class="tableBox">
      <el-table :data="toolList" border tooltip-effect="light" >
        ...
        <el-table-column label="操作" align="center" width="220px">
          <template #default="scope">
            ...
            <el-button v-if="roleInfo?.name === `管理员`" type="danger" size="small" @click="delFn(scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
