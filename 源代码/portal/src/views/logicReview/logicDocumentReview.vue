          ...
          if (resultData.status === `IN_PROGRESS` || resultData.status === `ERROR`) {
            sourceApi.getSourceCodeReviewProgress({ id: reviewId }).then(res1 => {
              if (res1.succ) {
                ...
                const rawFileRules1 = res1.content.fileRule || []
                const nullCount1 = rawFileRules1.filter(item => item == null).length
                const missingStatus1 = rawFileRules1.filter(item => item != null && item.status === undefined).length
                if (nullCount1 > 0 || missingStatus1 > 0) {
                  console.warn('[审查进度] fileRule数据异常: null元素数=', nullCount1, ', status缺失数=', missingStatus1, ', 原始数据=', rawFileRules1)
                }
                fileRuleLists.value = rawFileRules1.filter(item => item != null).map(item => ({
                  ...item,
            ...
            sourceApi.getSourceCodeReviewProgress({ id: reviewId }).then(res2 => {
              if (res2.succ) {
                ...
                const rawFileRules2 = res2.content.fileRule || []
                const nullCount2 = rawFileRules2.filter(item => item == null).length
                const missingStatus2 = rawFileRules2.filter(item => item != null && item.status === undefined).length
                if (nullCount2 > 0 || missingStatus2 > 0) {
                  console.warn('[审查完成] fileRule数据异常: null元素数=', nullCount2, ', status缺失数=', missingStatus2, ', 原始数据=', rawFileRules2)
                }
                fileRuleLists.value = rawFileRules2.filter(item => item != null).map(item => ({
                  ...item,
                  status: item.status ?? 3,
                  ruleSource: item.ruleSource ?? '',
                  ruleType: item.ruleType ?? '',
...
{language: [], selectStatus: ["1"], ruleType: [], ruleSource: [], desc: ""}
  ...
  const delRecordsFn = (item: any) => {
    ElMessageBox.confirm(`确定删除审查记录?`, `提示`, {
      ...
      .then(() => {
        sourceApi.logicReviewResultFileId(item).then(res => {
          if (res.succ) {
  ...
  const downloadFn = (item: any) => {
    ...
    sourceApi.logicReviewResultDetailList({"resultId": item.resultId})
      .then(res => {
        const arr: any[] = []
  ...
  const accept1 = `.v,.vh,.verilog,.vlg,.vhd,.vhdl,.zip`
  ...
  const handleBeforeAudio = async (file: any) => {
    ...
    const acceptTypes = [`v`, `vh`, `verilog`, `vlg`, `vhd`, `vhdl`, `zip`]
    ...
    const result = await sourceApi.logicUploadCheckFile({fileName,isTestFile:uploadForm.isTestFile})
    console.log('-result',result)
    if(result.succ){
      if(result.content.exist){
        console.log('上传参数-file',file)
        uploadForm.file = file
        uploadForm.fileId = result.content.fileId
        uploadRenameText.value = `该文件名已经存在，是否继续上传`
        console.log('上传参数-uploadForm', uploadForm)
  ...
  const uploadSubmit = async (formEl: any) => {
    ...
    await formEl.validate((valid: boolean, fields: any) => {
      if (valid) {
        ...
        sourceApi
          .logicUploadFile({ ...uploadForm, file: fd })
          .then(res => {
            loading.value = false
            if (res.succ) {
              // 上传后重新加载第一页数据
  ...
  const goReviewStatistics = () => {
    router.push({ path: `/logicStatistics` })
  }
  ...
  const gzlbFn = async () => {
    router.push({ path: `/logicRuleList` })
  }
  ...
  const viewVersionFn = (row: any) => {
    rowData.value = row
    sourceApi.logicFileVersionResult({ fileId: row.id }).then(res => {
      if (res.succ) {
        ...
        <div class="tableWrapper">
          ...
          <el-table-column label="操作" align="center" width="350" class-name="opt" fixed="right">
            <template #default="scope">
              ...
              <el-button :disabled="scope.row.status !== 'FINISHED'" v-loading="scope.row.onImportLoading" type="primary" size="small" class="btn" @click="downloadFn(scope.row)">下载报告</el-button>
          ...
          <el-form ref="uploadFormRef" style="max-width: 600px" :model="uploadForm" :rules="uploadRules" label-width="auto" class="demo-ruleForm" status-icon>
            ...
            <el-form-item prop="file">
              ...
              <div class="uploadFile">
                ...
                <p style="margin-top: 10px;">支持 .v,.vh,.verilog,.vlg,.vhd,.vhdl,.zip 类型文件</p>
              </div>
                <template #file="{ file }">
      ...
      <el-dialog v-model="showRuleConfirmModal" class="dialogCont dialogCont7" title="" :show-close="false" width="70%" :close-on-click-modal="false" :close-on-press-escape="false">
        <template #header>
          <div class="my-header">
            ...
            <div class="right">
              <el-icon @click="closeRuleConfirmModal" style="cursor: pointer; font-size: 20px;"><Close /></el-icon>
            </div>
          </div>
        </template>
          ...
          <div class="tableCont">
            <el-table ref="rulesListRef" border tooltip-effect="light" :data="fileRuleLists" height="100%">
              ...
              <el-table-column prop="status" label="审查结果" align="center" width="200px" class-name="status">
                <template #default="scope">
                  <div :class="{ tg: scope.row?.status === 1, pszb: scope.row?.status === 3, notg: scope.row?.status === 2 }">
                    {{ scope.row?.status === 1 ? `通过` : scope.row?.status === 2 ? `未通过` : `审查中` }}
                    <div v-show="scope.row?.status === 1" class="dg">
                      <el-icon><Select /></el-icon>
                    </div>
                    <div v-show="scope.row?.status === 2" class="ng">
                      <!-- <i class="iconfont icon-butongguo1" />   -->
                       <el-icon><Close /></el-icon>                
                    </div>
                    <div v-show="scope.row?.status === 3" class="dot">
                      <div />
                      <div />
                      <div />
                    </div>
