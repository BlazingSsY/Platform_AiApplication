<script setup lang="ts">
  ...
  import { logicReviewRuleAll ,logicReviewRuleSelect, logicReviewRuleDetails,logicReviewRuleMetadata} from "@/ajax/logicreview"
  ...
  const logicReviewRuleAllFn = ()=>{
    loading.value = true
    logicReviewRuleAll({pageSize:pages.pagesize,pageNum:pages.pageNum,filter:filter.value})
      .then(res => {
        loading.value = false
        if (res.succ) {
  ...
  const logicReviewRuleMetadataFn = ()=>{
    logicReviewRuleMetadata().then(res=>{
      if(res.succ){
  ...
  const choseFn = ()=>{
    ...
    logicReviewRuleSelect({ids:selectedIds}).then(res=>{
      if(res.succ){
  ...
  const exportFn = ()=>{
    ...
    logicReviewRuleAll({pageSize:999,pageNum:1})
      .then(res => {
        if (res.succ) {
  ...
  const detailFn = (item)=>{
    logicReviewRuleDetails({ids:[item.id]}).then(res=>{
      if(res.succ){
