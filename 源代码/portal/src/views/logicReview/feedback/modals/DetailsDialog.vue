...
import { logicFileVersionResult } from '@/ajax/logicreview'
...
const getFileDownloadUrl = (item) => {
  ...
  return `/logicreview/common/v1/storage/download/${item.fileId}?fileId=${item.fileId}&fileName=${encodeURIComponent(item.fileName)}`
}
...
const downloadFn = (item) => {
  if(item.fileId){
    window.open(`/logicreview/common/v1/storage/download/${item.fileId}?fileId=${item.fileId}&fileName=${encodeURIComponent(item.fileName)}`)
  }
}
