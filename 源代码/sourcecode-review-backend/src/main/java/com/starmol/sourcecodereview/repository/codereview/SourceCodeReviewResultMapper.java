...
import com.starmol.sourcecodereview.bean.vo.SourceCodeReviewHomeStatisticsDataItemVO;
...
public interface SourceCodeReviewResultMapper extends BaseMapper<SourceCodeReviewResult> {
    ...
    @Select({"<script>",
            ...
            "        review_time, ",
            "        status ",
            "    FROM dmsc_review_result where is_delete=0 ",
            "    ORDER BY file_id, review_time DESC ",
            ") r ",
            "INNER JOIN dmsc_file f ON r.file_id = f.id ",
    ...
    @Select({"<script>",
            ...
            ") r ",
            "INNER JOIN dmsc_file f ON r.file_id = f.id ",
            "LEFT JOIN (select result_id, count(DISTINCT rule_id) rule_count FROM dmsc_review_result_detail  where is_delete = 0 group by result_id) detail ON r.id = detail.result_id ",
            "LEFT JOIN urm_department d ON f.department_id = d.id ",
            "WHERE r.status = 2 AND f.is_delete = 0 AND f.file_name NOT LIKE 'test-%' ",
            "<if test='departmentIds != null and departmentIds.size() > 0'> ",
            "  AND f.department_id IN ",
    ...
    @Select({"<script>",
            ...
            " source_file_name,",
            " is_passed",
            " FROM dmsc_review_result_detail where is_delete=0 ORDER BY result_id, rule_id, source_file_name, is_passed ",
            ") detail ON r.id = detail.result_id ",
            "INNER JOIN dmsc_rule rule ON detail.rule_id = rule.id ",
            "WHERE r.status = 2 AND f.is_delete = 0 AND f.file_name NOT LIKE 'test-%' ",
            "<if test='departmentIds != null and departmentIds.size() > 0'> ",
            "  AND f.department_id IN ",
            "  <foreach collection='departmentIds' item='deptId' open='(' separator=',' close=')'> ",
