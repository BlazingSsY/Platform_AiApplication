...
import com.starmol.sourcecodereview.bean.bo.SourceCodeReviewResultDetailRuleBO;
...
public interface SourceCodeReviewResultDetailMapper extends BaseMapper<SourceCodeReviewResultDetail> {
    @Select({"<script>",
        ...
        "FROM dmsc_review_result_detail d ",
        "LEFT JOIN dmsc_rule r ON d.rule_id = r.id ",
        "WHERE d.is_delete = 0 ",
    ...
    @Select({"<script>",
            "SELECT r.name FROM ( SELECT DISTINCT ON (rule_id) rule_id, is_passed FROM dmsc_review_result_detail ",
