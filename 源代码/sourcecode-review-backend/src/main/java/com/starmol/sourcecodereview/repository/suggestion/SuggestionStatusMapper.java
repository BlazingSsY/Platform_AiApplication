...
public interface SuggestionStatusMapper extends BaseMapper<SuggestionStatus> {
     ...
     */
    @Select({"<script>",
            "SELECT a.*,u.name AS createUserName FROM code_yjfk_suggestion_status a LEFT JOIN urm_user u ON a.create_user = u.id AND a.is_delete = 0 AND u.is_delete = 0 where a.is_delete=0 ",
