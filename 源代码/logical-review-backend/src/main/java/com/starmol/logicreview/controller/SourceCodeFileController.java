...
import com.starmol.logicreview.bean.bo.PulverizeFileResultBO;
import com.starmol.logicreview.bean.dto.ChangeFileRecycleStatusDTO;
import com.starmol.logicreview.bean.dto.FileNameDTO;
import com.starmol.logicreview.bean.dto.SourceCodeFileDTO;
import com.starmol.logicreview.bean.vo.*;
...
@Slf4j
@RestController
@RequestMapping("/v1/logic-file")
@Tag(name = "逻辑文件管理", description = "逻辑文件相关接口")
public class SourceCodeFileController {
    ...
    @PostMapping
    @Operation(summary = "创建逻辑文件", description = "创建新的逻辑文件")
    public ResponseWrapper<SourceCodeFileVO> createSourceCodeFile(@RequestBody SourceCodeFileDTO sourceCodeFile) {
        try {
            ...
            log.error("创建逻辑文件失败", e);
            return ResponseWrapper.fail("创建逻辑文件失败: " + e.getMessage());
        }
    }
    ...
    @Operation(summary = "创建逻辑文件(直接上传文件)", description = "创建新的逻辑文件(检查后无同名文件,直接上传文件)")
    ...
    @PutMapping("/{id}")
    @Operation(summary = "更新逻辑文件", description = "根据ID更新逻辑文件")
    public ResponseWrapper<SourceCodeFileVO> updateSourceCodeFile(
            @Parameter(description = "逻辑文件ID") @PathVariable Long id,
        ...
        try {
            ...
            log.error("更新逻辑文件失败", e);
            return ResponseWrapper.fail("更新逻辑文件失败: " + e.getMessage());
        }
    }

    @GetMapping("/{id}")
    @Operation(summary = "获取逻辑文件", description = "根据ID获取逻辑文件详情")
    public ResponseWrapper<SourceCodeFileVO> getSourceCodeFile(
        ...
        try {
            ...
            if (result == null) {
                return ResponseWrapper.fail("逻辑文件不存在");
            }
            return ResponseWrapper.success(result);
        } catch (Exception e) {
            log.error("获取逻辑文件失败", e);
            return ResponseWrapper.fail("获取逻辑文件失败: " + e.getMessage());
        }
    }

    @GetMapping("/{fileId}/version")
    @Operation(summary = "获取逻辑文件所有版本", description = "根据ID获取逻辑文件所有版本")
    public ResponseWrapper<List<SourceCodeFileVersionVO>> getSourceCodeFileVersions(
            @Parameter(description = "逻辑文件ID") @PathVariable Long fileId) {
        try {
            List<SourceCodeFileVersionVO> result = sourceCodeFileVersionService.getSourceCodeFileVersionVOById(fileId);
            return ResponseWrapper.success(result);
        } catch (Exception e) {
            log.error("获取逻辑文件所有版本失败", e);
            return ResponseWrapper.fail("获取逻辑文件所有版本失败: " + e.getMessage());
        }
    }

    @GetMapping("/{fileId}/version-result")
    @Operation(summary = "获取逻辑文件所有版本和审查结果", description = "根据ID获取逻辑文件所有版本和审查结果")
    public ResponseWrapper<List<SourceCodeFileVersionAndResultVO>> getSourceCodeFileAllVersionAndResult(
        ...
        try {
            ...
            log.error("获取逻辑文件所有版本和审查结果失败", e);
            return ResponseWrapper.fail("获取逻辑文件所有版本和审查结果失败: " + e.getMessage());
        }
    }

    @GetMapping("/page")
    @Operation(summary = "分页查询逻辑文件详情", description = "分页查询逻辑文件详情，包括最新的审查结果")
    public ResponseWrapper<IPage<SourceCodeFileDetailVO>> getSourceCodeFilePage(
        ...
        try {
            ...
            log.error("分页查询逻辑文件失败", e);
            return ResponseWrapper.fail("分页查询逻辑文件失败: " + e.getMessage());
        }
    }

    @GetMapping("/recycle/page")
    @Operation(summary = "获取回收站逻辑文件分页", description = "获取回收站逻辑文件分页")
    public ResponseWrapper<IPage<SourceCodeFileVO>> getRecycledSourceCodeFilePage(
        ...
        try {
            ...
            log.error("获取回收站逻辑文件分页失败", e);
            return ResponseWrapper.fail("获取回收站逻辑文件分页失败: " + e.getMessage());
        }
    }

    @PostMapping("/is-recycle")
    @Operation(summary = "变更逻辑文件回收站状态", description = "变更逻辑文件回收站状态")
    public ResponseWrapper<Boolean> changeSourceCodeFileRecycleStatus(@RequestBody ChangeFileRecycleStatusDTO changeFileRecycleStatusDTO) {
        try {
            ...
            log.error("变更逻辑文件回收站状态失败", e);
            return ResponseWrapper.fail("变更逻辑文件回收站状态失败: " + e.getMessage());
        }
    }


    @DeleteMapping("/{id}")
    @Operation(summary = "删除逻辑文件", description = "根据ID删除逻辑文件")
    public ResponseWrapper<String> deleteCircuitFile(
        ...
        try {
            ...
            log.error("删除逻辑文件失败", e);
            return ResponseWrapper.fail("删除逻辑文件失败: " + e.getMessage());
        }
    }
