<script setup lang="ts">
  ...
  const { fileName,fileVersionName,id,auditSubmitTime, fileId, fileVersionId }: any = route.query
  ...
  const columnOrder = computed(() => {
    ...
    // }
    return [...baseOrder, `auditType`, `issueFeedback`]
    // return baseOrder
  })
  const columnMapping = {
    "ruleType": `规则类型`,
  ...
  const gotoReviewResult = () => { 
    if (fileObj.value) {
      ...
      router.push({
        "path": `/reviewResults`,
        "query": {
          "item": encodeURIComponent(JSON.stringify(fileObj.value))
        }
      })
    }
  }
      ...
      ElMessageBox({
        ...
        beforeClose: (action, instance, done) => {
          if (action === 'confirm') {
            ...
            if (value.length > 100) {
              ElMessage.warning('拒绝理由不能超过100字')
              return
            }
            ...
            >
              <template #default="scope">
                ...
                <template v-else-if="column.name === '审查意见'">
                  <span v-if="scope.row.isPassed !== 1">{{ scope.row[column.prop] }}</span>
