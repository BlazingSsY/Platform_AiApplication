...
import { sourceCodeFileVersionResult } from '@/ajax/sourcecodereview'
...
const getRepliesData = (fId) => {
  if (fId) {
    getReplies({ fId, pageSize: 9999, pageNumber: 1 })
    .then((res) => {
      if (res.succ)
...
const getFileDownloadUrl = (item) => {
  ...
  return `/sourcecodereview/common/v1/storage/download/${item.fileId}?fileId=${item.fileId}&fileName=${encodeURIComponent(item.fileName)}`
}
...
const downloadFn = (item) => {
  if(item.fileId){
    window.open(`/sourcecodereview/common/v1/storage/download/${item.fileId}?fileId=${item.fileId}&fileName=${encodeURIComponent(item.fileName)}`)
  }
}
