<script setup lang="ts">
  ...
  const crushFn = (item: any) => {
    ElMessageBox.confirm(`确定粉碎?`, `提示`, {
      ...
      .then(() => {
        sourceApi.deleteSourceCodeFile([item]).then(res => {
          if (res.succ) {
  ...
  const handleBeforeAudio = async (file: any) => {
    ...
    if (!(type == `h` || type == `c` || type == `zip`||type == `cpp` || type == `hpp` || type == `cxx`|| type == `java`)) {
    ...
    const reuslt = await sourceApi.sourceCodeUploadCheckFile(file.name)
    if (reuslt.succ && reuslt.content.exist) {
      uploadForm.file = file
      uploadForm.fileId = reuslt.content.fileId
    ...
    <div v-loading.fullscreen.lock="loading" class="documentReview" element-loading-background="rgba(122, 122, 122, 0.8)" :element-loading-text="loadingType === 1 ? `数据加载中...` : `文件上传中...`">
      ...
      <div class="tableBox">
        ...
        <div class="tableWrapper">
          ...
          <el-table-column prop="tel_file" label="代码文件" align="center" show-overflow-tooltip min-width="120px" />
          <el-table-column prop="departmentName" label="单位名称" align="center" min-width="100px" show-overflow-tooltip />
          <el-table-column prop="total_cases" label="文件数" align="center" width="120px" >
            <template #default="scope">
              {{ scope.row.total_cases || `-` }}
            </template>
          </el-table-column>
          <el-table-column prop="filesLine" label="行数" align="center" width="120px" />
          <el-table-column prop="useRuleSize" label="审查规则数" align="center" width="130px" />
          <el-table-column prop="questions" label="问题数量" align="center" width="120px" />
          ...
          <el-table-column prop="status" label="审查状态" align="center" width="155px" >
            <template #default="scope">
          ...
          <el-table-column label="操作" align="center" width="350" class-name="opt" fixed="right">
            <template #default="scope">
              ...
              <el-button
                v-else-if="scope.row.reviewTime && scope.row.status === 'ERROR'"
                class="btn"
                type="danger"                
                size="small"
                disabled
              ...
              <!--              删除记录-->
              <!--            </el-button>-->
              <el-button type="danger" size="small" class="btn" :disabled="scope.row.status == 'IN_PROGRESS'"  @click="crushFn(scope.row)">粉碎文件</el-button>
      ...
      <el-dialog v-if="uploadShow" v-model="uploadShow" class="dialogCont dialogCont3" title="" :show-close="false" width="600px" :close-on-click-modal="false" :close-on-press-escape="false">
        ...
        <div class="contBox">
          <el-form ref="uploadFormRef" style="max-width: 600px" :model="uploadForm" :rules="uploadRules" label-width="auto" class="demo-ruleForm" status-icon>
            <!-- <el-form-item label="涉密级别" prop="fileSecretLevelEnum">
              <el-select v-model="uploadForm.fileSecretLevelEnum" filterable allow-create default-first-option placeholder="请选择涉密级别" style="width: 400px">
            ...
            <div class="progress">
              ...
              <div class="label">               
               <div v-if="taskNum!=0" class="taskNum">
                    资源排队中，您前面有{<span style="color: #FF4D4F;">{{ taskNum }}</span>}位资源
                </div>
                <div v-else>
                <b>正在审查: </b>
                <el-tooltip :content="reviewProgress.fileName" placement="top">
                  <span class="review-file-name">{{ reviewProgress.fileName }}</span>
                </el-tooltip>
              </div>
              </div>
            </div>
      ...
      <el-dialog v-model="uploadRename" title="提示" width="500" top="36vh">
        <span>该文件名已经存在，将形成文件新版本,是否继续上传？</span>
        <template #footer>
          <div class="dialog-footer">
            <el-button @click="cancleFn"> 取消 </el-button>
            <el-button type="primary" @click="uploadRenameSubmit"> 上传 </el-button>
          </div>
        </template>
      </el-dialog>
