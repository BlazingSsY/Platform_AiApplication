...
public interface AnswerMapper extends BaseMapper<Answer> {
     ...
     */
    @Select({"<script>",
            "SELECT a.*,u.name AS createUserName FROM yjfk_answer a LEFT JOIN urm_user u ON a.create_user = u.id AND a.is_delete = 0 AND u.is_delete = 0 where a.is_delete=0 ",
            "<if test='fId != null'> ",
            "  AND a.f_id = #{fId} ",
            "</if> ",
