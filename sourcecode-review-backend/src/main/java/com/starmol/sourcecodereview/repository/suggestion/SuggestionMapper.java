...
public interface SuggestionMapper extends BaseMapper<Suggestion> {
     ...
     */
    @Select({"<script>",
            "SELECT a.*,u.name AS createUserName FROM code_yjfk_suggestion a LEFT JOIN urm_user u ON a.create_user = u.id AND a.is_delete = 0 AND u.is_delete = 0 where a.is_delete=0 ",
