...
import com.starmol.sso.server.constant.GlobalVariableToJunit;
...
import com.starmol.sso.server.util.JsonUtil;
...
@Slf4j
@Component
public class ApplicationManagementDataInitRunner implements ApplicationRunner {
	...
	private OauthClientToRedisBO getClient() {
		...
		oauthClientToRedisBO.setClientName("yunlian SSO management");
		oauthClientToRedisBO.setClientId(GlobalVariableToJunit.CLIENT_ID);
		oauthClientToRedisBO.setClientSecret("a8V0t0f7LusVZghwY3qDYMzcJ6RpSg3G");
		oauthClientToRedisBO.setClientUrl("^(http|https)://.*");
		oauthClientToRedisBO.setClientDesc("");
	...
	private OauthAccessTokenToRedisBO getAccessToken() {
		...
		oauthUserAttribute.setId(GlobalVariableToJunit.USER_ID);
		oauthUserAttribute.setLoginName(GlobalVariableToJunit.USERNAME);
