...
import com.starmol.sso.server.util.ExceptionUtil;
import com.starmol.sso.server.util.StringUtil;
import com.starmol.sso.server.util.response.R;
import com.starmol.sso.server.util.response.ResponseErrorObject;
...
public class ExceptionControllerAdvice {
	 ...
	 * OAuth API 业务异常
	 */
	@ExceptionHandler(OauthPasswordExpiredException.class)
	public Object handleOauthApiException(HttpServletRequest httpServletRequest, OauthPasswordExpiredException e) {
		String message = e.getMessage();
		ResponseProduceTypeEnum responseProduceTypeEnum = e.getResponseProduceTypeEnum();
		if (null == responseProduceTypeEnum) {
			responseProduceTypeEnum = ResponseProduceTypeEnum.JSON;
		...
		return getResponseObject(e, HttpStatus.OK, httpServletRequest, message, responseProduceTypeEnum, pagePath);
	}

	@ExceptionHandler(OauthRestException.class)
	public ResponseWrapper<String> handleOauthApiException(OauthRestException e) {
		log.error("Got Error: {}", e.getMessage());
		return ResponseWrapper.failed(e.getMessage(), e.getCode());
	}
